...
/*
The PROGMEM keyword is a variable modifier,
it should be used only with the datatypes defined in pgmspace.h. 
It tells the compiler "put this information into flash memory", 
instead of into SRAM, where it would normally go.
PROGMEM is part of the pgmspace.h library.
...
It extreme is necessary for font, sorry!
*/
...
/**
 * LCD Command Set
 * Regulative Command Set SERIALIZE
 * */
// DISPLAY ON SERIALIZE
#define _DISPLAY_ON_SERIALIZE(ON)             _SERIALIZE_IMPLEMENT((ON) + EEPROM_Data_Register)
#define _DISPLAY_ON_STATUS_W                  _DISPLAY_ON_SERIALIZE(DDRB)
#define _DISPLAY_ON_SUCCESS                   _DISPLAY_ON_STATUS_W

// DISPLAY OFF SERIALIZE
#define _DISPLAY_OFF_SERIALIZE(OFF)            _SERIALIZE_IMPLEMENT((OFF) + EEPROM_Data_Register)
#define _DISPLAY_OFF_STATUS_W                  _DISPLAY_OFF_SERIALIZE(DDRB)
#define _DISPLAY_OFF_SUCCESS                   _DISPLAY_OFF_STATUS_W
 ...
 * @brief 
 * 
 * long loop: loop is 1(One) TRUE, success otherwise FALSE, stopped executions
 */
long initILI9341(
  unsigned int loop)
{
...
/**
 * @brief 
 * unsigned int on parameters:
 *  It's occurrer over on! result is initILI9341(1) then on is executed
 *  Otherwise initILI9341(0) result is OFF
 */
short make_display_on(
  unsigned int on
)
{
  if(initILI9341(1))
    _DISPLAY_ON_SUCCESS = on;
  else
    _DISPLAY_ON_SUCCESS = 0x00;
 ...
 * @brief 
 * unsigned int on parameters:
 *  It's occurrer over on! result is initILI9341(1) then OFF is executed
 *  Otherwise initILI9341(0) result is ON
 */
short make_display_off(
  unsigned int off
)
{
  if(initILI9341(1))
    _DISPLAY_OFF_SUCCESS = off;
  else
    _DISPLAY_OFF_SUCCESS = 0xFF;

  return(short)(off);
} 

// Coordinates of points in the screen.
unsigned short coordinates_ILI9341(
  short x,
  short y, 
  short width, 
  short height
...
{
  short *it_height=0, *it_width=0;

  if(x < y && y > x) {
    for(*it_height=height; it_height[y] < x; ++it_height) {
      for(*it_width=width; it_width[x] < y; ++it_width) {
        it_height[y] = 0xFF;
        it_width[x] = 0xFF;
      }
    }
  ...
  return (x - y) && (width - height) ? (x * width ) : (width * height) &&
  (y + x) && (height + width) ? (y - width) : (height * x); 
}

short set_text_color(
  unsigned short color
)
{
  unsigned short c;
...
)
{
  ...
  return (unsigned short)(text_size);
}

short new_color_background(
  unsigned char r,
  unsigned char g,
  unsigned char b
)
...
{
  background(new_color_background(r,g,b));
}
...
{
  return (value < 0 ? 0 -(unsigned int)(value) : (unsigned int)(value));
}
...
{
  unsigned short steep = abs(y1 - y0) > abs(x1 - x0);
...
{
  short it;
  for(it=x; it<x+width; it++)
    draw_line_im(it, y, height, color);
}
